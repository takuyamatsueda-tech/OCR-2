...
export const PdfViewer: React.FC<PdfViewerProps> = ({ file, onLoadSuccess, currentPage }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gray-100 relative">
        <div
            className={`absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-gray-900 text-white px-4 py-2 rounded-md shadow-lg transition-all duration-300 ease-in-out
            ${copyMessage ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}
        >
            {copyMessage}
        </div>
        <TextLayerStyles />
