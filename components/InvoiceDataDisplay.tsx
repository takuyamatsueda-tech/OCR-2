...
const EditableDataRow: React.FC<{ 
    ...
    type?: string;
    disabled?: boolean;
}> = ({ label, fieldValue, onChange, onShowHistory, type = 'text', disabled = false }) => (
  <div className="grid grid-cols-3 gap-4 py-2 border-b items-center">
    <dt 
      className="text-sm font-medium text-gray-500 col-span-1 flex items-center"
    >
      {label}
...
}> = ({item, field, originalIndex, type = 'text', disabled, handleItemChange, showHistory}) => {
...
export const InvoiceDataDisplay: React.FC<DocumentDataDisplayProps> = ({ data, fileName, currentPage, disabled, onUpdateData, fieldConfig }) => {
