...
import { ProcessResult, DocumentType, DocumentData, LineItem, DocumentConfig, DocumentFieldConfig, FieldValue, OutputConfig, OutputFieldConfig } from './types';
import { DEFAULT_DOCUMENT_CONFIG } from './config/documentSettings';
import { OutputSettingsModal } from './components/OutputSettingsModal';
...
const applyOutputFormatting = (value: any, instruction: string | undefined): string => {
    const strValue = String(value ?? '');
    if (!instruction) return strValue;
    return instruction.replace('{value}', strValue);
};
...
const App: React.FC = () => {
  ...
  const [documentConfig, setDocumentConfig] = useState<DocumentConfig>(() => {
    try {
      ...
      console.error("Failed to load documentConfig from localStorage", error);
  ...
  const [outputConfig, setOutputConfig] = useState<OutputConfig>(() => {
    try {
      ...
      console.error("Failed to load outputConfig from localStorage", error);
      return {};
    }
  });
  
  const [isSettingsModalOpen, setIsSettingsModalOpen] = useState(false);
  const [isOutputSettingsModalOpen, setIsOutputSettingsModalOpen] = useState(false);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [editingDocType, setEditingDocType] = useState<DocumentType | null>(null);

  const synchronizeConfigs = useCallback((docConfig: DocumentConfig) => {
    let newOutputConfig: OutputConfig = JSON.parse(JSON.stringify(outputConfig));
    let hasChanged = false;

    Object.keys(docConfig).forEach(docType => {
        if (!newOutputConfig[docType]) {
            newOutputConfig[docType] = [];
            hasChanged = true;
        }

        const docFields = docConfig[docType];
        const outputFields = newOutputConfig[docType];
        const outputFieldKeys = new Set(outputFields.map(f => f.key));

        // Add new fields from docConfig to outputConfig
        docFields.forEach(docField => {
            if (!outputFieldKeys.has(docField.key)) {
                outputFields.push({
                    key: docField.key,
        ...
        const docFieldKeys = new Set(docFields.map(f => f.key));
        // Remove fields from outputConfig that no longer exist in docConfig
        newOutputConfig[docType] = outputFields.filter(outputField => docFieldKeys.has(outputField.key));
        if (newOutputConfig[docType].length !== outputFields.length) {
          hasChanged = true;
        }
  ...
  useEffect(() => {
    try {
      ...
      synchronizeConfigs(documentConfig);
    } catch (error) {
  ...
  useEffect(() => {
    try {
        ...
        console.error("Failed to save output config to localStorage", error);
    }
  }, [outputConfig]);
  ...
  const handleSaveOutputConfig = (docType: DocumentType, newFields: OutputFieldConfig[]) => {
      setOutputConfig(prev => ({
          ...prev,
          [docType]: newFields
      }));
      setIsOutputSettingsModalOpen(false);
  }
  ...
  const handleOpenOutputSettings = (type: DocumentType, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingDocType(type);
    setIsOutputSettingsModalOpen(true);
  };
  ...
  const handleExportAllCsv = () => {
    const confirmedResults = processResults.filter(r => r.status === 'confirmed' && r.data?.document_type);
    if (confirmedResults.length === 0) return;

    const masterHeaderConfig = new Map<string, OutputFieldConfig>();
    
    confirmedResults.forEach(result => {
        const docType = result.data!.document_type as DocumentType;
        const configForDocType = outputConfig[docType] || [];
        configForDocType.forEach(field => {
            if (field.enabled && !masterHeaderConfig.has(field.key)) {
                masterHeaderConfig.set(field.key, field);
            }
        });
    });

    const sortedMasterHeaderConfig = Array.from(masterHeaderConfig.values());

    const headers = ['ファイル名', 'ドキュメントタイプ', ...sortedMasterHeaderConfig.map(c => c.label)];
    const rows: string[][] = [];
    ...
    for (const result of confirmedResults) {
        ...
        const docOutputConfig = outputConfig[docType] || [];
        // FIX: Explicitly type the Map to prevent type inference issues where `configMap.get()` returns `unknown`.
        const configMap = new Map<string, OutputFieldConfig>(docOutputConfig.map(f => [f.key, f]));

        const items = doc.items && doc.items.length > 0 ? doc.items : [{} as LineItem];

        for (const item of items) {
            let row: string[] = [fileName, doc.document_type];
            sortedMasterHeaderConfig.forEach(c => {
                let value;
                const fieldConf = documentConfig[docType]?.find(f => f.key === c.key);
                if (fieldConf?.isItemField) {
                    value = item[c.key]?.value;
                } else {
                    value = (doc[c.key] as FieldValue<string | number>)?.value;
                }

                const formattedValueWithDocRule = applyFormatting(value, fieldConf?.outputFormat || 'none');
                
                const outputConf = configMap.get(c.key);
                const finalFormattedValue = applyOutputFormatting(formattedValueWithDocRule, outputConf?.formatInstruction);
                row.push(finalFormattedValue);
            });
            rows.push(row);
        }
    }
  ...
  if (view === 'selection') {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4 font-sans">
        ...
        {isOutputSettingsModalOpen && editingDocType && (
            <OutputSettingsModal
                docType={editingDocType}
                docConfig={documentConfig[editingDocType]}
                outputConfig={outputConfig[editingDocType] || []}
                onClose={() => setIsOutputSettingsModalOpen(false)}
            ...
            {Object.keys(documentConfig).map(type => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={(e) => handleOpenOutputSettings(type as DocumentType, e)} title="出力設定" className="text-gray-500 hover:text-blue-600 p-2 rounded-full transition-colors">
                      <OutputIcon />
                    </button>
                    <button onClick={(e) => handleOpenSettings(type as DocumentType, e)} title="読み取り項目設定" className="text-gray-500 hover:text-blue-600 p-2 rounded-full transition-colors">
                      <SettingsIcon />
                    </button>
